...
const ATTR_SESSION_KEY = 'emsj_session_id_v1'
...
function readStoredAttribution() {
  ...
  try {
    return JSON.parse(window.localStorage.getItem(ATTR_STORAGE_KEY) || '{}')
  } catch {
    return {}
  }
}
...
function ensureSessionId() {
  ...
  const existing = window.sessionStorage.getItem(ATTR_SESSION_KEY)
  if (existing) return existing
  const generated = `sid-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  window.sessionStorage.setItem(ATTR_SESSION_KEY, generated)
  return generated
}

function captureAttributionFromCurrentVisit() {
  if (typeof window === 'undefined') return
  ...
  const hasCampaignData = Object.keys(captured).length > 0
  const touch = {
    ...captured,
    page: window.location.pathname,
    at: new Date().toISOString(),
    source: captured.utm_source || (captured.gclid ? 'google-ads' : 'direct'),
  }

  const next = {
    sessionId: ensureSessionId(),
    firstTouch: stored.firstTouch || touch,
    lastTouch: hasCampaignData ? touch : (stored.lastTouch || touch),
  }
...
function resolveCtaContext(eventOrContext) {
  if (typeof eventOrContext === 'string') return slugify(eventOrContext)
  const el = eventOrContext?.currentTarget
  if (!el) return 'whatsapp'
  const fromAttrs = el.id || el.getAttribute('aria-label') || el.dataset.waContext
  if (fromAttrs) return slugify(fromAttrs)
  const fromText = (el.textContent || '').trim()
  return slugify(fromText || 'whatsapp')
}

function buildTrackedWhatsAppUrl(rawUrl, ctaContext = 'whatsapp') {
  if (typeof window === 'undefined') return rawUrl
  const url = new URL(rawUrl, window.location.origin)
  if (!url.hostname.includes('wa.me')) return rawUrl

  const attribution = getAttributionSnapshot()
  const lastTouch = attribution.lastTouch || {}

  if (!url.searchParams.get('text')) {
    url.searchParams.set('text', 'Hola, quiero consultar por un repuesto.')
  }

  url.searchParams.set('ref_page', window.location.pathname)
  url.searchParams.set('ref_cta', slugify(ctaContext))
  url.searchParams.set('ref_sid', attribution.sessionId)
  ...
  if (lastTouch.gclid) url.searchParams.set('gclid', lastTouch.gclid)

  return url.toString()
}

function enrichWhatsAppLinksOnPage() {
  if (typeof window === 'undefined') return
  const links = document.querySelectorAll('a[href*="wa.me/"]')
  links.forEach((link) => {
    if (!link.dataset.waBaseHref) {
      link.dataset.waBaseHref = link.getAttribute('href') || WHATSAPP
    }
    const ctaContext = link.id || link.dataset.waContext || link.getAttribute('aria-label') || link.textContent || 'whatsapp'
    link.href = buildTrackedWhatsAppUrl(link.dataset.waBaseHref, ctaContext)
  })
}

function trackWhatsAppConversion(eventOrContext = 'whatsapp') {
  const ctaContext = resolveCtaContext(eventOrContext)
  const attribution = getAttributionSnapshot()
  const lastTouch = attribution.lastTouch || {}

  if (eventOrContext?.currentTarget?.href) {
    eventOrContext.currentTarget.href = buildTrackedWhatsAppUrl(eventOrContext.currentTarget.href, ctaContext)
  }

  if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
    window.gtag('event', 'whatsapp_click', {
      event_category: 'lead',
      event_label: ctaContext,
      page_path: window.location.pathname,
      session_id: attribution.sessionId,
      traffic_source: lastTouch.source || 'direct',
      utm_campaign: lastTouch.utm_campaign || '(none)',
    })
...
function openTrackedWhatsApp(baseUrl, ctaContext) {
  const trackedUrl = buildTrackedWhatsAppUrl(baseUrl, ctaContext)
  trackWhatsAppConversion(ctaContext)
      ...
      {menuOpen && (
        <div className="absolute top-full mt-3 left-0 right-0 glass-dark border-subtle rounded-3xl p-4 flex flex-col gap-3 md:hidden">
          ...
          <a
            ...
            onClick={() => { trackWhatsAppConversion('mobile-menu-cta'); setMenuOpen(false); }}
          >
...
function FloatingWA() {
  return (
    <a
      ...
      className="fixed right-6 z-50 flex items-center gap-2 pl-3 pr-4 py-3 sm:w-14 sm:h-14 sm:p-0 sm:justify-center rounded-full bg-[#25D366] shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-[0_0_30px_rgba(37,211,102,0.4)]"
    >
      <span
        className="absolute inset-0 rounded-full bg-[#25D366] opacity-50 animate-ping pointer-events-none"
        style={{ animationDuration: '2.5s' }}
      />
      <svg viewBox="0 0 24 24" fill="white" className="w-7 h-7 relative flex-shrink-0">
      ...
      <span className="text-white font-bold text-sm sm:hidden relative">Consultanos</span>
    </a>
  )
}
...
export default function App() {
  useEffect(() => {
    captureAttributionFromCurrentVisit()

    let rafId = 0
    const scheduleLinkEnrichment = () => {
      if (rafId) return
      rafId = window.requestAnimationFrame(() => {
        rafId = 0
        enrichWhatsAppLinksOnPage()
      })
    }

    scheduleLinkEnrichment()
    const observer = new MutationObserver(scheduleLinkEnrichment)
